...
deny[msg] {
	...
	criteria_0(request)
	msg := "Denied by NeuVector rule #1002: [item.spec.initContainers[_].lifecycle.postStart.tcpSocket.host containsAll localhost]"
}
...
deny[msg] {
	request := _get_input("get")
	criteria_1(request)
	msg := "Denied by NeuVector rule #1002: [item.spec.containers[_].image containsAny redis:latest,nginx:latest,sbomscanner]"
}
